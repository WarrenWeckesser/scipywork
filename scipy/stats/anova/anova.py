...
if __name__ == "__main__":
    # Example from page 454 of Richard Johnson "Miller & Freund's Probability
    ...
    # Example 12.1 from Zar (fifth ed.), pages 251-255
    conc = np.array(
    ...
    # From Table 23.4, Draper & Smith (third ed.)
    rates = np.array(
        [[[4, 6], [6, 4], [13, 15], [12, 12]],
         [[11, 7], [13, 15], [15, 9], [12, 14]],
         [[5, 9], [9, 7], [13, 13], [7, 9]]])

    # From Sokal & Rohlf (fourth ed.), Box 11.1
    ...
    dr = np.array(
        [[[17, 21, 49, 54], [64, 48, 34, 63], [62, 72, 61, 91]],
         [[33, 37, 40, 16], [41, 64, 34, 64], [56, 62, 57, 72]]])
    ...
    acttext = """
     ...
     49 male    ss        3.110
     """
    actfields = list([w.split()[1:] for w in acttext.strip().splitlines()])
    act = {}
    for s, g, a in actfields:
        key = (s, g)
        cell = act.setdefault(key, [])
        cell.append(float(a))
    actdata = [[act[s, g] for g in ['ff', 'fs', 'ss']] for s in ['female', 'male']]
